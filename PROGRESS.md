...
| **Not yet done** | — | Running pixel-vet v2 and T7 on real pixels and alternate reductions (both need per-target MAST downloads, outside this session's catalog-only authorization); the batman/emcee transit fit and SED sanity check (§4.6) that would populate `fit_posteriors`; TRICERATOPS itself (deferred by owner decision); review-queue UI (§8) |
