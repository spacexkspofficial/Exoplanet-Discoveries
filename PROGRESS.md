...
| **Not yet done** | — | Running pixel-vet v2 and T7 on real pixels and alternate reductions (both need per-target MAST downloads, outside this session's catalog-only authorization); TRICERATOPS/fit/packet assembly (§4.6, deferred by owner decision to an interface, which is also not yet built); review-queue UI (§8) |
